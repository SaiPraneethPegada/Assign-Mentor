...
router.patch("/students", async (req, res) => {
    ...
    try {
        ...
        if (mentorData) {
            const assign = await MentorsDetails.findOneAndUpdate(
                { mentorId },
                { $set: { studentAssigned: studentsArray } }
            );
            // console.log(assign);
...
router.patch("/mentor", async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            message:
                "Updated mentor to respective student , updated in old mentor and new mentor studentAssigned list",
            newMentor,
        });
    } catch (error) {
